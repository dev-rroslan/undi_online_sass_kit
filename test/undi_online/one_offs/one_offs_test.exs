defmodule UndiOnline.OneOffsTest do
  use UndiOnline.DataCase

  alias UndiOnline.OneOffs
  alias UndiOnline.OneOffs.Task

  import UndiOnline.OneOffsFixtures
  ...
  describe "tasks" do
    ...
    test "execute/0 executes the test task" do
      ...
      assert [%Task{module: "UndiOnline.OneOffsTest.TestTask"}] = OneOffs.list_tasks()
      assert_received :task_executed
    end

    test "execute/0 does not execute the test task if its in the database" do
      task_fixture(%{module: "UndiOnline.OneOffsTest.TestTask"})
